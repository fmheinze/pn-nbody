...
2. After the simulation has finished, the output should be located in the `./output` directory. You can visualize the output with the included Python scripts in the `./tools/viz` directory, e.g.:
```
python3 ./tools/viz/trajectory_anim.py ./output/test_figure_eight/output_pos.dat
```
...
Use the `output` parameter to select which output files are created. Separate them with spaces:

```text
output = position velocity energy merger
```

To monitor a selected body pair, add `orbit_X_Y` to `output`, where `X` and `Y` are distinct 1-based
...
orbit_1_2 = a_newtonian e_newtonian
orbit_1_3 = separation a_r e_r energy_binary
```
