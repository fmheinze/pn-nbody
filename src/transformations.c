...
void transform_to_com(int* bodies, int num_bodies, struct ode_params* params, double* w, double* w_transform) {
    ...
    allocate_vector(&R_COM, params->num_dim);
    allocate_vector(&V_COM, params->num_dim);
    ...
    for (int a = 0; a < num_bodies; a++) {
        for (int i = 0; i < params->num_dim; i++) {
            R_COM[i] += params->masses[bodies[a]] * w[params->num_dim * bodies[a] + i];
            V_COM[i] += w[array_half + params->num_dim * bodies[a] + i];
        }
    }
    ...
    for (int a = 0; a < params->num_bodies; a++) {
        for (int i = 0; i < params->num_dim; i++) {
            w_transform[params->num_dim * a + i] = w[params->num_dim * a + i] - R_COM[i];
            w_transform[array_half + params->num_dim * a + i] = w[array_half + params->num_dim * a + i] - V_COM[i];
        }
    }
