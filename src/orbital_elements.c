...
void total_angular_momentum_com(int* bodies, int num_bodies, double* w, struct ode_params* params, double* L) {
    ...
    for (a = 0; a < num_bodies; a++) {
        index_r = params->dim * bodies[a];
        index_p = array_half + params->dim * bodies[a];
...
double eccentricity(int* bodies, double* w, struct ode_params* params) {
    ...
    if (params->dim == 3) {
        double* L;
        allocate_vector(&L, params->dim);
        total_angular_momentum_com(bodies, 2, w, params, L);
        L2 = dot_product(L, L, params->dim);
        free_vector(L);
    }
    ...
    params_bodies.dim = params->dim;
    params_bodies.num_bodies = 2;
    allocate_vector(&w_bodies, 2 * params_bodies.dim * 2);
    ...
    for (int a = 0; a < 2; a++) {
        for (int i = 0; i < params->dim; i++) {
            w_bodies[a * params->dim + i] = w[bodies[a] * params->dim + i];
            w_bodies[2 * params->dim + a * params->dim + i] = w[array_half + bodies[a] * params->dim + i];
        }
    }
